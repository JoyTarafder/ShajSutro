      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-7 py-5 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{category ? "Update category details" : "Create a new product category"}</p>
          </div>
        ...
        <form onSubmit={submit} className="px-7 py-5 space-y-4">
          <div>
            ...
            {form.name && (
              <p className="text-[11px] text-gray-400 mt-1.5">
                Slug: <span className="font-mono text-gray-500">{form.name.toLowerCase().replace(/\s+/g, "-").replace(/[^\w-]/g, "")}</span>
              </p>
            )}
          </div>
          <div>
          ...
          <div>
            ...
            <input
              name="image" type="url" value={form.image} onChange={set}
              placeholder="https://…"
              className="form-input w-full"
            />
            {form.image && (
...
function CategoriesContent() {
  ...
  const deletingCategory = categories.find((c) => c._id === deleteId);
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <p className="text-sm text-slate-400 font-medium">{categories.length} categor{categories.length !== 1 ? "ies" : "y"}</p>
        <button
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-slate-400">
          ...
          <button onClick={() => setModalCategory("new")} className="mt-3 text-violet-600 text-sm hover:underline font-bold">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {categories.map((cat) => (
            <div key={cat._id} className="bg-white rounded-2xl border border-slate-100 shadow-soft overflow-hidden group hover:shadow-md transition-shadow">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8">
            ...
            <p className="text-sm text-gray-500 mb-1">
              Delete <span className="font-semibold text-gray-800">{deletingCategory?.name}</span>?
            </p>
            <p className="text-xs text-gray-400 mb-7">Categories with associated products cannot be deleted.</p>
...
export default function CategoriesPage() {
  return <AdminAuthGuard><CategoriesContent /></AdminAuthGuard>;
}
