  ...
  if (state.items.length === 0) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6 text-center py-28">
        ...
        <h1 className="text-2xl font-semibold text-charcoal-950 mb-3">Your cart is empty</h1>
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-12">
        <h1 className="text-3xl font-semibold text-charcoal-950 mb-2 tracking-tight">Shopping Cart</h1>
        <p className="text-charcoal-400 text-sm mb-12 font-light">{totalItems} {totalItems === 1 ? "item" : "items"}</p>
        ...
        <div className="grid lg:grid-cols-3 gap-14">
          <div className="lg:col-span-2">
            ...
            <div className="divide-y divide-charcoal-100">
              {state.items.map((item, index) => (
                <div key={`${item.product.id}-${item.size}-${item.color}-${index}`} className="py-7 flex gap-5">
                  <Link href={`/product/${item.product.id}`} className="relative w-24 h-32 flex-shrink-0 rounded-2xl overflow-hidden bg-warm-50 shadow-soft">
                    <Image
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between gap-4">
                      <div>
                        <Link href={`/product/${item.product.id}`} className="text-base font-medium text-charcoal-900 hover:text-charcoal-600 transition-colors duration-300 line-clamp-1">
                          {item.product.name}
                        </Link>
                        <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5">
                          <span className="text-sm text-charcoal-400 font-light">Size: {item.size}</span>
                          <span className="text-sm text-charcoal-400 font-light">Color: {item.color}</span>
                        </div>
                      </div>
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      <div className="flex items-center border border-charcoal-200 rounded-lg">
                        ...
                        <span className="w-9 text-center text-sm font-medium text-charcoal-900">{item.quantity}</span>
                        <button
                          onClick={() => updateQuantity(item.product.id, item.size, item.color, item.quantity + 1)}
                          className="w-9 h-9 flex items-center justify-center text-charcoal-400 hover:text-charcoal-900 transition-colors"
                        >
            ...
            <div className="mt-7 pt-7 border-t border-charcoal-100">
              <Link href="/shop" className="btn-secondary text-sm inline-flex items-center gap-2 group">
          ...
          <div>
            <div className="bg-warm-50 rounded-2xl p-7 sticky top-28 border border-warm-100">
              <h2 className="text-lg font-semibold text-charcoal-950 mb-7">Order Summary</h2>
              ...
              <div className="mt-5 flex items-center justify-center gap-2 text-xs text-charcoal-300">
                ...
                <span className="font-light">Secure SSL encrypted checkout</span>
              </div>
            </div>
          </div>
        </div>
